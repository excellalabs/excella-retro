...
module.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("templates/close.html",
    "<nav-bar></nav-bar><div class=row-fluid><div class=jumbotron><h1>Thank you!</h1><p>You've just completed a retrospective using our app. We're continually improving this app and would love to receive your feedback below.</p><p><form role=form name=sendFeedbackForm ng-submit=sendFeedback()><error-field model=validation></error-field><div class=form-group><label for=feedback>Feedback</label><input class=form-control id=feedback placeholder=\"What can we improve?\" ng-model=feedback></div><button type=submit class=\"btn btn-primary\">Send feedback</button></form></p></div></div><div class=\"row-fluid text-center\"><h2>or</h2></div><div class=\"row-fluid text-center\"><h2><a href=\"/#/\">Start or join a retrospective</a></h2></div><branding></branding>");
}]);
})();
...
module.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("templates/join.html",
    "<nav-bar></nav-bar><div class=row-fluid><div class=\"col-md-offset-3 col-md-6 col-md-offset-3\"><form role=form ng-submit=joinBoard()><error-field model=validation></error-field><div class=form-group><label for=userName>Name</label><input class=form-control id=userName placeholder=\"Enter your name\" ng-model=user></div><button type=submit class=\"btn btn-primary\" value=\"Join board\">Join retrospective</button></form></div></div><branding></branding>");
}]);
})();
...
module.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("templates/modal.html",
    "<div class=modal-header><h3 class=modal-title>{{title}}</h3></div><div class=modal-body>{{body}}</div><div class=modal-footer><button class=\"btn btn-primary\" ng-click=ok()>OK</button> <button class=\"btn btn-warning\" ng-click=cancel() ng-if=hasCancel>Cancel</button></div>");
}]);
})();
...
module.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("templates/directives/scrumMasterTools.html",
    "<div class=row-fluid><div class=\"col-md-12 no-padding\" id=boardTools><div class=\"panel panel-danger\"><div class=panel-heading><div class=row-fluid style=\"height: 18px\"><div class=col-xs-11><h3 class=panel-title>Facilitator Tools</h3></div><div class=\"col-xs-1 no-padding\"><span id=collapsor class=\"glyphicon glyphicon-chevron-up pull-right\"></span> </div></div></div><div class=panel-body><div class=\"row-fluid collapse in\"><div class=\"col-xs-6 col-sm-5 col-md-4 col-lg-4\"><div class=row-fluid>Invite Participants</div><clas s=row-fluid><div class=col-xs-2><a class=\"btn btn-default glyphicon glyphicon-link\" target=_blank ng-href={{participantJoinLink(board.id)}} title=\"Copy link to send participants\"></a></div><div class=col-xs-2><a class=\"btn btn-default glyphicon glyphicon-envelope\" ng-href=\"{{participantMailToLink(board.id, board.title)}}\" title=\"Send email to participants\"></a></div><div class=col-xs-8><form name=rc><span class=input-group><span class=\"input-group-addon glyphicon glyphicon-pencil\"></span><input class=form-control name=retroCode ng-model=boardId readonly></span></form></div></clas></div></div><div class=\"col-xs-6 col-sm-5 col-md-4 col-lg-4\"><div>Admin</div><a class=\"btn btn-warning glyphicon glyphicon-link\" ng-href=\"{{scrumMasterAccessLink(board.id, board.scrumMasterKey)}}\" title=\"Copy link to access as Facilitator\"></a> <a class=\"btn btn-danger glyphicon glyphicon-off\" ng-click=closeRetro()></a></div></div><div class=row-fluid ng-if=\"board.phase != phases.summary\"><div class=col-xs-12><div><a class=\"btn btn-primary glyphicon glyphicon-play\" ng-click=goToNextPhase()></a> <span>{{actionText}}</span></div></div></div></div></div></div>");
}]);
})();
...
module.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("templates/directives/whatWentWell.html",
    "<div class=row-fluid><div class=\"col-md-12 no-padding\" id=board><alert type=info><strong>What Went Well?</strong><span ng-show=!isScrumMaster>Once your Facilitator initiates feedback gathering you may share your feedback.</span></alert><span ng-show=\"isScrumMaster && board.phase == phases.wellFeedbackStarted\" class=help-block>Ask your team to provide feedback now. Time box the exercise and encourage everyone to share feedback.</span><add-feedback ng-if=\"board.phase == phases.wellFeedbackStarted\" feedback-context=\"What Went Well\" board-id=board.id type=well></add-feedback><div class=row-fluid ng-if=\"board.phase == phases.wellFeedbackCompleted\"><span ng-show=isScrumMaster class=help-block>Group feedback together by dragging and dropping one on to another. The initial item being dropped on will define the header for that group of feedback. You may re-arrange by un-grouping.</span><folder-holder class=\"col-xs-11 col-sm-11 col-md-12 col-lg-12\" on-change=sendThemes readonly lists=board.wellFeedback></folder-holder></div></div></div>");
}]);
})();
